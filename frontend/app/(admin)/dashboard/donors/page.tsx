...
export default function DonorsPage() {
  ...
  // Debug: Log donor data to see the structure
  console.log('Donors data:', donors.slice(0, 3)); // Log first 3 donors for debugging
  ...
  const getFilteredDonorsByTab = () => {
    switch (activeTab) {
      case "organization":
        // Organization donors: those with donorType === 'ORGANIZATION'
        return donors.filter(d => d.donorType === 'ORGANIZATION');
      case "unregistered":
        // Unregistered donors: walk-in/event donors who haven't claimed their account yet
        // Exclude organizations from this list since they have their own tab
        return donors.filter(d => 
          d.user?.isVerified === false && 
                          ...
                          {d.donorType === 'ORGANIZATION' ? (
                            ...
                            d.user?.isVerified ? (
                              ...
                              <div className="flex items-center gap-1">
                                ...
                                <span className="text-[10px] bg-green-100 text-green-700 px-1.5 py-0.5 rounded font-medium">Org</span>
                              </div>
                            )
                          ) : !d.user?.isVerified ? (
                            <span className="text-[10px] text-orange-600 font-medium">⚠ Unregistered</span>
