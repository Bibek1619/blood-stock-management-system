...
export default function BloodSearchPage() {
  ...
  const updateMapOverlays = useCallback(() => {
    ...
    console.log('Updating map overlays with', donorsArray.length, 'donors');
  ...
  useEffect(() => {
    if (!isLoading && donorsArray.length > 0 && mapObjRef.current) {
      console.log('Donors loaded, updating map with', donorsArray.length, 'donors');
      updateMapOverlays();
    }
  }, [isLoading, donorsArray.length, updateMapOverlays]);
    ...
    <div className="min-h-screen bg-slate-50 p-4 sm:p-6">
      <div className="max-w-[1300px] mx-auto">
        ...
        {sheetDonor && (() => {
          ...
          // Get coordinates for distance calculation
          let coords = sheetDonor.latitude && sheetDonor.longitude
